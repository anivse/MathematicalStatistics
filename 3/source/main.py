...
def diagram_with_mode(data, mode):
    ...
    plt.fill_between(index_not_mode, np.array(data_not_mode) - eps, np.array(data_not_mode) + eps, color='royalblue', alpha=0.3)
    ...
    plt.xlabel('n')
    plt.ylabel('mV')
    plt.savefig('diagram_with_mode.png')
...
if __name__ == "__main__":
    data = pd.read_csv('channel_2_600nm_2mm.csv', sep=';', encoding='cp1251')
    data = data['мВ']
    interval_data = []
    for i in range (0, len(data)):
